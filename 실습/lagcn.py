...
class Model(nn.Module):
    def __init__(self, num_class=12, num_point=18, num_person=1, graph="graph.ucla.Graph", graph_args=dict(), examplar="graph.cls_examplar.CLSExamplar", examplar_args=dict(), in_channels=3,
                 drop_out=0, adaptive=True):
        super(Model, self).__init__()
        # graph = np.load("what_will_[J]_act_like_when_[C]-with-punctuation copy" + '.npy').reshape(-1)[:3888].reshape(12,18,18)
        if graph is None:
            raise ValueError()
        else:
            ...
            self.examplar = Examplar(topo_str="what_will_[J]_act_like_when_[C]-with-punctuation",base_dir='cls_matrix_NWUCLA')
