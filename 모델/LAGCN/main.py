...
class Processor():
    ...
    def __init__(self, arg):
        ...
        if arg.phase == 'train':
            if not arg.train_feeder_args['debug']:
                ...
                self.extra_writer = SummaryWriter(os.path.join(arg.model_saved_name, 'extra'), 'extra')
            else:
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data, label, index, file_name) in enumerate(process):
            self.global_step += 1
            with torch.no_grad():
    ...
    def eval(self, epoch, save_score=False, loader_name=['test'], wrong_file=None, result_file=None):
        ...
        for ln in loader_name:
                ...
                if wrong_file is not None or result_file is not None:
                    ...
                    for i, x in enumerate(predict):
                        ...
                        if x != true[i] and wrong_file is not None:
                            f_w.write(str(file_name[i]) + ':' + str(index[i]) + ',' + str(x) + ',' + str(true[i]) + '\n')
            ...
            self.extra_writer.add_scalar('loss', loss, self.global_step)
            self.extra_writer.add_scalar('acc', accuracy, self.global_step)
            score_dict = dict(
